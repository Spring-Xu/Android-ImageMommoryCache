package cjstar.com.imagefilecache.cache;
...
public class LRUFileCache implements FileCache {
    ...
    private LRUFileCache() {
        this.options = new FileCacheOptions.Builder()
                //TODO if SDCard is exsit
                .setCacheRootPath(Environment.getExternalStorageDirectory().getPath() + "/FileCache")
    ...
    public File getDiskFile(String key) {
        ...
        Log.d(TAG, "getDiskFile:" + key);

        if(file!=null&&file.exists()){
            updateFileTime(file);
            Log.d(TAG, "getDiskFile find");
    ...
    public void removeDiskFile(String key) {
        Log.d(TAG, "removeDiskFile key:" + key);
